export default function OneCouponCard({ coupon, isReceived = false, onReceive, onDetailClick }) {
	return (
		<li className="oc w-full" data-code="275">
			...
			<div 
				className="coupons__card overflow-hidden flex flex-col w-full border border-slate-300 rounded-lg bg-white"
			...
			>
				<div className="coupons__header flex p-2 items-center bg-slate-200/20">
					<button 
						...
						className={`zzimbtn js_zzimbtn flex items-center justify-center px-2.5 py-1 text-sm text-[rgb(244,63,94)] border border-[rgb(244,63,94)] rounded-full
							${isReceived ? 'active bg-[rgb(244,63,94)] text-white' : '[&.active]:bg-[rgb(244,63,94)] [&.active]:text-white [&.active_svg]:fill-white [&.active_.empty-heart]:hidden [&.active_.filled-heart]:inline-block'}
						`}
					>
						{isReceived ? (
							<>
				...
				<div className="flex flex-col items-center p-1.5">
					...
					<div className="coupons__info relative flex flex-col items-center">
						<span className="coupons__name line-clamp-2 text-center font-bold leading-tight" title={coupon.name}>
							{coupon.name}
						</span>
